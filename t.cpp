...
//    std::stringstream t;
    ...
    if (hf!=INVALID_HANDLE_VALUE){
        ...
        /*//tempp << x[0] << "/output" << "/distrib_000001.dat";              //поток для записи названия файла!! номер чекпоинта!!
        //tempp >> temps;                                      //перекидываем его в строку, т.к в char нельзя
        //tempp.str("");                                       //очищаем поток,
        //tempp.clear();
        temps = x[0] + "/output" + "/distrib_000001.dat";                                   //чтобы можно было его использовать
